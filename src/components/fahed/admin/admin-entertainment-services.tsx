...
import {
  ...
  Edit3, Save, X, ChevronDown, ChevronUp, Gamepad2,
  Film, CreditCard, Upload, Package, Sparkles,
} from 'lucide-react';
...
const defaultEntertainmentSubSections: SubSection[] = [
  { id: 'sub-games', name: 'الألعاب', icon: '', order: 0, categoryIds: ['games'] },
  { id: 'sub-cards', name: 'البطاقات الرقمية', icon: '', order: 1, categoryIds: ['cards'] },
...
export default function AdminEntertainmentServices() {
  const {
  ...
  const [subSections, setSubSections] = useState<SubSection[]>(defaultEntertainmentSubSections);
  ...
  const [newProvider, setNewProvider] = useState({ name: '', color: '#8B5CF6', categoryId: 'games', inputLabel: '', inputType: 'text' as 'phone' | 'text', inputPrefix: '', icon: '' });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(subRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        // Initialize defaults
        setSubSections(defaultEntertainmentSubSections);
        defaultEntertainmentSubSections.forEach(sub => {
          try { set(ref(database, `adminSettings/entertainmentSubSections/${sub.id}`), sub); } catch {}
        });
      }
    });
    return () => unsubscribe();
  ...
  const getProvidersForSub = (sub: SubSection) => {
    return entertainmentProviders.filter(p => {
      if (sub.categoryIds.includes(p.categoryId)) return true;
      // For streaming sub-section, also match by provider name patterns
      if (sub.categoryIds.includes('streaming')) {
        const nameLower = p.name.toLowerCase();
        if (streamingProviderNames.some(sp => nameLower.includes(sp))) return true;
      }
      return false;
  ...
  const handleAddSubSection = () => {
    ...
    const sub: SubSection = {
      ...
      categoryIds: newSubSection.categoryIds ? newSubSection.categoryIds.split(',').map(s => s.trim()) : ['games'],
    };
    try {
      set(ref(database, `adminSettings/entertainmentSubSections/${id}`), sub);
      addAuditEntry(`تم إضافة قسم فرعي ${sub.name} في الخدمات الترفيهية`);
    } catch {}
  ...
  const handleDeleteSubSection = (sub: SubSection) => {
    try {
      remove(ref(database, `adminSettings/entertainmentSubSections/${sub.id}`));
  ...
  const onAddProvider = (subSectionId: string) => {
    ...
    setNewProvider({ name: '', color: '#8B5CF6', categoryId: 'games', inputLabel: '', inputType: 'text', inputPrefix: '', icon: '' });
  ...
  const getSubSectionIcon = (sub: SubSection) => {
    if (sub.icon && sub.icon.startsWith('data:')) {
      return <img src={sub.icon} alt={sub.name} className="w-8 h-8 rounded-lg object-cover" />;
    }
    if (sub.categoryIds.includes('games')) {
      return <Gamepad2 size={16} color="#8B5CF6" />;
    }
    if (sub.categoryIds.includes('cards')) {
  ...
  return (
    <motion.div key="entertainmentServices" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
      {/* Header */}
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h2 className="text-base font-bold" style={{ color: isDark ? '#FFF' : '#1a1a1a' }}>الخدمات الترفيهية</h2>
          <p className="text-[10px]" style={{ color: isDark ? '#888' : '#999' }}>إدارة الألعاب والبطاقات الرقمية ومنصات البث</p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showAddSubSection && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="rounded-2xl p-4 space-y-3 overflow-hidden" style={cardStyle}>
            ...
            <input type="text" placeholder="معرفات الفئات (مفصولة بفواصل: games, cards, streaming)" value={newSubSection.categoryIds} onChange={(e) => setNewSubSection({ ...newSubSection, categoryIds: e.target.value })} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={inputStyle} dir="ltr" />
            <div>
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={handleAddSubSection} className="w-full py-3 rounded-xl text-sm font-bold text-white" style={{ background: '#8B5CF6' }}>إضافة القسم</motion.button>
          </motion.div>
        )}
      </AnimatePresence>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="p-3 space-y-3">
                    ...
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => {
                      ...
                      const firstCategory = sub.categoryIds[0] || 'games';
                      setNewProvider(prev => ({ ...prev, categoryId: firstCategory, inputLabel: firstCategory === 'games' ? 'Player ID' : firstCategory === 'cards' ? 'البريد الإلكتروني' : 'الحساب', inputType: 'text', inputPrefix: '' }));
                    }}
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="rounded-xl p-3 space-y-2 overflow-hidden" style={{ background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.02)' }}>
                          ...
                          <select value={newProvider.categoryId} onChange={(e) => setNewProvider({ ...newProvider, categoryId: e.target.value })} className="w-full px-3 py-2 rounded-lg text-xs outline-none" style={inputStyle}>
                            {sub.categoryIds.map(catId => (
                              <option key={catId} value={catId}>{getCategoryIdLabel(catId)}</option>
                            ))}
                          ...
                          <div className="flex gap-2">
                            <motion.button whileTap={{ scale: 0.95 }} onClick={() => onAddProvider(sub.id)} className="flex-1 py-2 rounded-lg text-[10px] font-bold text-white" style={{ background: '#8B5CF6' }}>إضافة</motion.button>
                                ...
                                <div className="flex items-center gap-2 flex-1">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                      <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: `${provider.color}15`, color: provider.color }}>{getCategoryIdLabel(provider.categoryId)}</span>
                                      <span className="text-[9px]" style={{ color: isDark ? '#666' : '#AAA' }}>{providerProducts.length} منتج</span>
                                    </div>
                                  </div>
                                </div>
                                <div className="flex items-center gap-1.5">
                                  <button onClick={() => { setShowAddProduct(showAddProduct === provider.id ? null : provider.id); setNewProduct(prev => ({ ...prev, providerId: provider.id })); }} className="w-6 h-6 rounded flex items-center justify-center" style={{ background: 'rgba(139,92,246,0.1)' }}>
                            ...
                            {showAddProduct === provider.id && (
                              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-3 py-2 overflow-hidden" style={{ background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.01)' }}>
                                <div className="space-y-2">
                                  ...
                                  <div className="flex gap-2">
                                    <motion.button whileTap={{ scale: 0.95 }} onClick={onAddProduct} className="flex-1 py-1.5 rounded-lg text-[10px] font-bold text-white" style={{ background: '#8B5CF6' }}>إضافة منتج</motion.button>
                                  ...
                                  {editingProduct === product.id ? (
                                    ...
                                    <>
                                      <div>
                                        ...
                                        <div className="flex items-center gap-1.5 mt-0.5">
                                          <span className="text-[10px] font-bold" style={{ color: '#8B5CF6' }}>{product.price.toLocaleString()} {currencySymbols[product.currency]}</span>
                    ...
                    {subProviders.length === 0 && (
                      <div className="flex flex-col items-center py-4">
                        <Gamepad2 size={24} strokeWidth={1.5} color={isDark ? '#333' : '#DDD'} />
      ...
      {subSections.length === 0 && (
        <div className="flex flex-col items-center py-8">
          <Gamepad2 size={40} strokeWidth={1.5} color={isDark ? '#333' : '#DDD'} />
