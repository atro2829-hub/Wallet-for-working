...
import { Mail, Lock, User, Eye, EyeOff, ArrowLeft, ShieldCheck, Phone } from 'lucide-react';
...
export default function AuthScreen() {
  ...
  // Register fields - Step 2 (phone, optional)
  const [regPhone, setRegPhone] = useState('');

  // OTP field
  const [otp, setOtp] = useState(['', '', '', '']);

  // Store Firebase UID for OTP step
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        setUser({
          ...
          cardIssuedAt: userData.cardIssuedAt || '',
          governorate: userData.governorate || '',
          theme: userData.theme || 'light',
        });
      } else {
        // User exists in Auth but not in RTDB - create record
  ...
  const handleRegisterStep2 = async () => {
    ...
    try {
      ...
      // Also register in Prisma DB via API
      try {
        ...
        // Non-critical, Firebase is primary
      }
  ...
  const handleOtpVerify = async () => {
    ...
    // After OTP, set the user in store
    try {
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 'register-step1' && (
            ...
            >
              ...
              <p className="text-xs mb-3" style={{ color: isDark ? '#888' : '#AAA' }}>
          ...
          {/* REGISTER STEP 2 - Phone (Optional) */}
          {step === 'register-step2' && (
            <motion.div
              key="register-step2"
              initial={{ x: 300, opacity: 0 }}
            ...
            >
              ...
              <p className="text-xs mb-3" style={{ color: isDark ? '#888' : '#AAA' }}>
                الخطوة 2 من 2 - رقم الهاتف (اختياري)
              </p>
              ...
              <div className="flex flex-col items-center mb-6">
                ...
                >
                  <Phone size={32} strokeWidth={1.5} color="#E60000" />
                </div>
                <p
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full py-3 rounded-2xl flex items-center justify-center gap-2 text-sm font-medium transition-all active:scale-[0.98] disabled:opacity-50"
                style={{
                  background: isDark ? '#222' : '#F0F0F0',
                  color: isDark ? '#AAA' : '#888',
                }}
              >
                <span>تخطي - بدون رقم هاتف</span>
              </button>
            </motion.div>
          )}
              ...
              <div className="flex flex-col items-center">
                ...
                {generatedUserId && (
                  <div
                    ...
                    style={{ background: 'rgba(230,0,0,0.1)' }}
                  >
                    <p className="text-xs" style={{ color: isDark ? '#AAA' : '#888' }}>
                      رقم حسابك
                    </p>
                    <p
                      className="text-lg font-bold"
